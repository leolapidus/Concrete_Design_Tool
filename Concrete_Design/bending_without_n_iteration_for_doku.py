...
def bending_without_n_iteration(model, values, concrete_type, exp):
    ...
    for i, ele in enumerate(model.elements):
        if type(ele)==BeamColumnElement:
            ...
            z = (1-s*k_a)*values.static_usable_height(ele.h, exp)
            ...
            if m_rds > m_ed:
                ...
                while diff > 0.001:
                    ...
                    arg = (m_ed)/(alpha_r*fcd*ele.b*values.static_usable_height(ele.h,exp)**2*(1-s*k_a))

                    eps_c_2 = arg*eps_s_1/(arg-1)
